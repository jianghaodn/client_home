<template>
	<div class="diy_details_box diy_div_commodity_category">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "commodity_category",
					source_field: "commodity_category_id",
					source_id: this.obj["commodity_category_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/commodity_category/set?commodity_category_id=' + _this.obj["commodity_category_id"], {
								praise_len
							}, (res) => {
								if(res.result){
