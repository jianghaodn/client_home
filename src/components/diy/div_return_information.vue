		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "return_information_id",
					source_id: this.obj["return_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/return_information/set?return_information_id=' + _this.obj["return_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
