<template>
	<div class="diy_details_box diy_div_distribution_information">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "distribution_information_id",
					source_id: this.obj["distribution_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/distribution_information/set?distribution_information_id=' + _this.obj["distribution_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
